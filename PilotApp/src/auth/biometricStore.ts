...
const SERVICE = 'com.pilotapp.descope.biometric';
...
async function runPrompt(allowDeviceCredentials: boolean): Promise<{ ok: boolean }> {
  const { success } = await rnBiometrics.simplePrompt({
    promptMessage: 'Sign in to Pilot App',
...
export function showBiometricUnavailableAlert(label: string, osMessage: string): void {
  Alert.alert(
    `Enable ${label}`,
    `${osMessage}\n\nPilot App uses ${label} to verify that it is you when you sign in. You can turn it on in Settings.`,
    [
