...
// the iOS Associated Domains entitlement, an Apple Team ID registered in the
// Descope Console, and a hosted AASA — deliberately out of scope for this
// pilot. The sign-in screen uses FlowView precisely because it has no passkey
// step in it.
...
import { markDeviceTrusted } from '../auth/deviceTrust';
 ...
 * (ASWebAuthenticationSession on iOS / Custom Tabs on Android). Because the
 * flow runs on Descope's own hosted domain, the passkey is a *web* passkey tied
 * to that domain, so the app needs no Associated Domains entitlement, no hosted
 * apple-app-site-association, and no Team/bundle matching.
 *
 * Two modes:
 *   - 'signin' (default, from Welcome) — unauthenticated sign-in.
 ...
 *
 * Claims: the session this returns carries the member's custom claims like any
 * other, because they come from the project's JWT Template rather than from
 * anything the sign-in flow does — see src/auth/claims.ts.
 *
...
export default function PasskeyScreen({ navigation, route }: Props) {
  ...
  const runPasskeyFlow = async () => {
    ...
    try {
      ...
      // passkey to the current account instead of starting a new sign-in.
      ...
      if (!resp.ok) {
        ...
        setError(
          (resp.error?.errorDescription ||
            (isAddMode ? 'Could not add passkey.' : 'Passkey sign-in failed.')) + code,
        );
        return;
      }
      ...
      if (resp.data) {
        try {
          ...
          if (!hadSession) {
            // A passkey binds a credential to this device, so the device is
            // trusted from here on and password sign-in won't demand an OTP.
            const loginId = resp.data.user?.email || resp.data.user?.loginIds?.[0] || '';
            if (loginId) {
        ...
        setError(
          `Passkey sign-in returned no session. Check that ${AUTH_REDIRECT_URL} ` +
      ...
      setError(
        err?.message || (isAddMode ? 'Could not add your passkey.' : 'Passkey sign-in failed.'),
      );
    } finally {
      setBusy(false);
    }
      ...
      {!configured ? (
        <View style={styles.body}>
          ...
          <Text style={styles.bodyText}>
            Set PASSKEY_SIGNIN_FLOW_ID and PASSKEY_ADD_FLOW_ID (and your Descope
            Project ID) in src/config/index.ts, then build those flows. See
            docs/descope-signin-flow-setup.md §6.
          </Text>
        </View>
      ) : (
        <View style={styles.body}>
