...
| **App privacy lock** | `src/auth/AppLock.tsx` — while signed in, covers the UI in the app switcher and re-asks for biometrics (with PIN/passcode fallback) when the app returns to the foreground. Only engages when biometrics is available; always offers a "Sign out" escape. |
