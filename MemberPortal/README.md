...
| **App privacy lock** | `src/auth/AppLock.tsx` — while signed in, switching away to another app arms a lock, and switching back re-asks for biometrics (with PIN/passcode fallback) before revealing the UI. Ignores transient interruptions (Control Center, notifications, the auth sheet), so it never covers the foreground. Only engages when biometrics is available; always offers a "Sign out" escape. |
